...
def word_length_sorted(words):
    ...
    for word in words:
        num_chars = len(word)
        if num_chars in words_by_length:
            words_by_length[num_chars].append(word)
        else:
            words_by_length[num_chars] = [word]

    # https://stackoverflow.com/questions/1217251/python-sorting-a-dictionary-of-lists
    print sorted(words_by_length.items(), key=lambda e: (e[0], e[1].sort()))
...
def translate_to_pirate_talk(phrase):
    ...
    return ' '.join([english_to_pirate_dict[word] if word in english_to_pirate_dict.keys() else word for word in phrase])
